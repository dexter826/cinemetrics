...
const Dashboard: React.FC = () => {
  ...
  const handleDelete = async (docId: string) => {
    if (window.confirm("Are you sure you want to delete this movie from your history?")) {
      try {
      ...
      {!TMDB_API_KEY && (
         <div className="bg-yellow-500/10 border-b border-yellow-500/20 p-2 text-center">
            <p className="text-yellow-500 text-sm flex items-center justify-center gap-2">
               ...
               TMDB API Key is missing. Search will not work. Add it to constants.ts or .env.
            </p>
         </div>
      )}
      ...
      <main className="max-w-screen-xl mx-auto px-4 md:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <StatsCard 
            ...
            subValue={`${stats.totalMinutes.toLocaleString()} total minutes`}
          />
          {/* Action Card */}
          <button 
          ...
          >
             <div>
                <p className="text-white/80 text-sm font-medium mb-1">Add to collection</p>
                <h3 className="text-2xl font-bold text-white">Log Movie</h3>
             </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
             <div className="flex items-baseline gap-3">
               <h2 className="text-xl font-semibold text-gray-200">Watch History</h2>
               <span className="text-sm text-gray-500">{movies.length} entries</span>
             </div>
                  ...
                  <div className="flex items-center space-x-2 bg-surface p-1 rounded-xl border border-white/5 self-start sm:self-auto">
                      <button
                        ...
                        className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                          sortBy === 'date' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
                        }`}
                      >
                        <Calendar size={14} />
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                          sortBy === 'title' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
                        }`}
                      >
                        <Type size={14} />
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                          sortBy === 'runtime' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
                        }`}
                      >
                        <Clock size={14} />
            ...
            <div className="border-2 border-dashed border-white/10 rounded-3xl p-12 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-medium text-white">
                  {searchQuery ? "No matching movies found" : "No movies yet"}
                </h3>
                <p className="text-gray-500 max-w-xs mx-auto">
                  {searchQuery ? "Try adjusting your search query." : "Start building your personal cinema history by adding your first movie."}
                </p>
              </div>
              {!searchQuery && (
                <button 
